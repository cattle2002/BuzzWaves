...
//Minio:
//EndPoint: 172.26.148.239:9000
//AccessKeyID: admin
//SecretAccessKey:  admin123
//UseSSL: false
//BucketName: BuzzWaves
func GetMinioEndpoint() string {
	m, ok := config.Config.Minio["EndPoint"].(string)
	if !ok {
		panic(errors.New("获取配置信息失败"))
	} else {
		return m
...
func GetMinioAccessKeyID() string {
	m, ok := config.Config.Minio["AccessKeyID"].(string)
	if !ok {
		panic(errors.New("获取配置信息失败"))
	} else {
		return m
...
func GetMinioSecretAccessKey() string {
	m, ok := config.Config.Minio["SecretAccessKey"].(string)
	if !ok {
		panic(errors.New("获取配置信息失败"))
	} else {
		return m
...
func GetMinioSecretUseSSL() bool {
	m, ok := config.Config.Minio["UseSSL"].(bool)
	if !ok {
		panic(errors.New("获取配置信息失败"))
	} else {
		return m
...
func GetMinioSecretBucketName() string {
	m, ok := config.Config.Minio["BucketName"].(string)
	if !ok {
		panic(errors.New("获取配置信息失败"))
	} else {
		return m
